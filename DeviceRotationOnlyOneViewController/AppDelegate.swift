...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, supportedInterfaceOrientationsFor window: UIWindow?) -> UIInterfaceOrientationMask {
        if let rootViewController = self.topViewControllerWithRootViewController(window?.rootViewController) {
            ...
            if (rootViewController.isKind(of: MainViewController.self)) {
                // Only allow portrait for this kind of controllers
                return UIInterfaceOrientationMask.portrait
            } else {
                // Unlock landscape view orientations for all other view controllers
